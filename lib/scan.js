...
const { getTokenValueLamports } = require("./jupiter");
const { log } = require("./log");
const { RENT_LAMPORTS, PROTECTED_MINTS, VALUE_THRESHOLD_LAMPORTS } = require("../config");
...
async function classifyOne(a) {
  ...
  if (meta.closeAuthority) {
    ...
    log(`💰 查询 ${a.mintStr.slice(0, 10)}… 价值…`);
    valueLamports = await getTokenValueLamports(a.mintStr, a.amountRaw);
    if (valueLamports !== null && valueLamports >= VALUE_THRESHOLD_LAMPORTS) {
      category = "valuable";
      reason = `有价值（约 ${(Number(valueLamports) / 1e9).toFixed(6)} SOL），跳过`;
    } else {
      category = "burnable";
    }
  }
