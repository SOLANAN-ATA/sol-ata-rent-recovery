...
const { scanVolumeAccumulators } = require("./pumpVolume");
...
// 超大钱包保护：单钱包代币账户数超过此上限时，只处理前 N 个（1G 内存机器防 JS 堆 OOM）
...
async function classifyOne(a) {
  ...
  // NFT 识别：0 位小数 + 持有恰 1 个 = 不可分割的唯一资产（Metaplex NFT / pNFT / 域名 NFT 都符合）。
  // 归为独立「nft」分类，默认不自动销毁，需用户在页面核对价值后勾选销毁（误烧防护）。
  const isNft = a.decimals === 0 && a.amountRaw === "1";
  let category;
  let reason = null;
  ...
  if (meta.closeAuthority) {
    ...
    category = "nft";
    reason = "疑似 NFT：请核对价值，确认是垃圾再勾选销毁";
  } else {
    // 不做价值检测：有余额的非 NFT 币归为可销毁（页面已提示客户先自行转走有价值的币）
    category = "burnable";
  }
  return { ...a, category, reason, meta, valueLamports, isNft };
}

/** 内部版：返回带 PublicKey 的分类结果（供 redeem 使用） */
async function buildClassification(ownerPk, capture = {}) {
  mintCache.clear(); // 每次扫描独立缓存，防跨钱包累积导致内存泄漏（mintCache 是模块级全局 Map）
  log("🔍 查询代币账户中…");
  let accounts = await getTokenAccounts(ownerPk);
  ...
  if (accounts.length > MAX_ACCOUNTS) {
    ...
    log(`⚠️ 账户数 ${totalAccounts} 超过上限 ${MAX_ACCOUNTS}，仅处理前 ${MAX_ACCOUNTS} 个（防 OOM）`);
  }
  capture.truncated = truncated;
  capture.totalAccounts = totalAccounts;
...
async function scanWallet(ownerPk) {
  const capture = {};
  const [items, balanceLamports, volume] = await Promise.all([
    buildClassification(ownerPk, capture),
    getBalance(ownerPk),
    scanVolumeAccumulators(ownerPk),
  ]);
  const summary = {
    total: items.length,
    empty: 0,
  ...
  const list = items.map((it) => {
    ...
    if (recoverable) {
      ...
      summary.recoverableLamports += it.lamports || RENT_LAMPORTS;
    }
    // NFT 的潜在租金单独累计（需用户勾选销毁后才真正可退）
    if (it.category === "nft") {
      summary.nftLamports += it.lamports || RENT_LAMPORTS;
    }
    return {
      mint: it.mintStr,
      ...
      recoverableLamports: (recoverable || it.category === "nft") ? (it.lamports || RENT_LAMPORTS) : 0,
    };
  });
  summary.recoverableSol = summary.recoverableLamports / 1e9;
  summary.nftSol = summary.nftLamports / 1e9;
  // Pump.fun 累加器账户（押金直接释放给用户，不走平台中转，单独列出）
  const volumeList = volume.map((v) => ({
    account: v.account.toBase58(),
    program: v.program.toBase58(),
    label: v.label,
    lamports: v.lamports,
    sol: v.sol,
  }));
  return {
    ...
    totalAccounts: capture.totalAccounts,
    maxAccounts: MAX_ACCOUNTS,
  };
}

module.exports = { scanWallet, buildClassification, analyzeMint, MAX_ACCOUNTS, NATIVE_MINT: null };
