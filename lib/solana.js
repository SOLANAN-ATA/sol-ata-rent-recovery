...
const BREAK_MS = 10 * 60 * 1000; // 持续失败/配额耗尽：10 分钟冷却
const SHORT_BREAK_MS = 15 * 1000; // 瞬时 429 限流：15 秒短冷却（别因一次突发把好节点熔断 10 分钟）
function isDead(url) {
...
async function rpcCallWithUrl(url, body, timeoutMs = 20000) {
  ...
  return new Promise((resolve) => {
    ...
    const req = https.request({
      ...
      res.on("data", (c) => (d += c));
      res.on("end", () => {
        // 429 = 瞬时限流：短冷却（突发后很快恢复），别把好节点长时间熔断
        if (res.statusCode === 429) { markDead(url, SHORT_BREAK_MS); resolve(null); return; }
        try {
          const j = JSON.parse(d);
          const msg = (j && j.error && j.error.message) || "";
          if (/rate.?limit|too many requests|429/i.test(msg)) markDead(url, SHORT_BREAK_MS);
          else if (/quota|exceeded/i.test(msg)) markDead(url);
          resolve(j);
        } catch { resolve(null); }
      });
    });
    ...
    programs.map(async ([tag, programId]) => {
      ...
      for (const a of (list && list.value) || []) {
        ...
        arr.push({
          ...
          program: programId,
          tag,
          // 账户当前 lamports = 实际押金（rent-exempt 金额）。
          // Token-2022 带扩展的账户（如 170 字节）租金高于标准 ATA（165 字节），
          // 关户时实际退回的是全部 lamports，故用真实值而非写死的 RENT_LAMPORTS。
...
/** 从 fromKp 转 lamports 给 toPk，返回签名（平台转净额给用户用） */
async function transferSol(fromKp, toPk, lamports) {
  ...
  tx.add(SystemProgram.transfer({ fromPubkey: fromKp.publicKey, toPubkey: toPk, lamports }));
  tx.sign(fromKp);
  const sig = await broadcastTransaction(tx.serialize().toString("base64"));
  if (!sig) throw new Error("所有 RPC 发送失败");
  return sig;
}

module.exports = { conn, rpcCall, getTokenAccounts, getAccountInfo, getBalance, broadcastTransaction, getSignatureStatusAll, checkRpcHealth, transferSol };
