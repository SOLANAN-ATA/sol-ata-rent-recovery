...
async function buildRedeemTransactions(userPk, platformPk, opts = {}) {
  ...
  const valuable = items.filter((it) => it.category === "valuable");
  if (!targets.length) {
    return { txs: [], targetCount: 0, rentLamports: 0, feeLamports: 0, netLamports: 0, rentSol: 0, feeSol: 0, netSol: 0, skippedValuable: valuable.length };
  }
  ...
  return {
    ...
    feeSol: totalFee / 1e9,
    netSol: totalNet / 1e9,
    skippedValuable: valuable.length,
  };
}
