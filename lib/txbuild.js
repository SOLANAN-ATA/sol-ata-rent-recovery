...
const { buildClassification, MAX_ACCOUNTS } = require("./scan");
const { buildCloseVolumeIx } = require("./pumpVolume");
...
// 累加器押金+返现：签名模式也收 10% 手续费（2026-09-01 起，与批量模式一致；此前免费当卖点）
const VOLUME_FEE_RATE = 0.10;
 ...
 * @returns {{ chunks, perChunkNet, targetCount, totalRent, totalFee, totalNet }}
...
async function classifyAndChunk(userPk, platformPk, opts = {}) {
  const capture = {};
  const items = opts.items || (await buildClassification(userPk, capture));
  // 用户勾选要销毁的 NFT 账户地址（误烧防护：NFT 默认不销毁，只有用户明确勾选才烧）
  const selectedNfts = new Set(opts.selectedNfts || []);
  // 处理 empty（空）+ burnable（有余额非 NFT 可烧）+ 用户勾选的 NFT；保护名单/不可赎回/nft 已在分类里排除
  const targets = items
    .filter((it) => {
      if (it.category === "empty" || it.category === "burnable") return true;
      if (it.category === "nft" && selectedNfts.has(it.account.toBase58())) return true;
      return false;
    })
    .map((it) => ({ ...it, burn: it.category === "burnable" || it.category === "nft" }));
  if (!targets.length) {
    return { chunks: [], perChunkNet: [], targetCount: 0, totalRent: 0, totalFee: 0, totalNet: 0, truncated: capture.truncated, totalAccounts: capture.totalAccounts, maxAccounts: MAX_ACCOUNTS };
  }
  ...
  const perChunkNet = chunks.map((c) => c.reduce((s, it) => s + (it.lamports || RENT_LAMPORTS), 0) - c.length * FEE_LAMPORTS);
  const totalRent = targets.reduce((s, it) => s + (it.lamports || RENT_LAMPORTS), 0);
  ...
  return { chunks, perChunkNet, targetCount: targets.length, totalRent, totalFee, totalNet, truncated: capture.truncated, totalAccounts: capture.totalAccounts, maxAccounts: MAX_ACCOUNTS };
}
...
async function buildChunkTx(chunk, userPk, platformPk) {
  ...
  const rentLamports = chunk.reduce((s, it) => s + (it.lamports || RENT_LAMPORTS), 0);
...
module.exports = { classifyAndChunk, buildChunkTx, buildVolumeTx };

/**
 * 构造 Pump.fun 累加器关户交易（只 CloseUserVolumeAccumulator）。
 * 关键结论（2026-08-27 simulate 验证）：Close 会把累加器全部 lamports
 * （押金 + 累积返现，返现物理存在累加器账户里）一次性释放给 user，
 * 无需先 ClaimCashback。且 pAMMBay 的 ClaimCashback 账户布局不同（需 quote_mint），
 * 会报 AccountOwnedByWrongProgram(3007)，所以统一只 Close。
 * 押金直接释放给 user（Close 的 close target = user），不走平台中转，无需 forward。
 * @param {Array<item>} volumeItems scanVolumeAccumulators 的返回（带 PublicKey 的 account/program/globalState）
 * @param {PublicKey} userPk 用户钱包（fee payer + 收款人 + 授权人）
 * @returns {{ serialized, accountCount, totalLamports, totalSol }}
 */
async function buildVolumeTx(volumeItems, userPk, platformPk) {
  const bh = await rpcCall("getLatestBlockhash", [{ commitment: "confirmed" }]);
  const blockhash = bh && bh.value && bh.value.blockhash;
  if (!blockhash) throw new Error("获取 blockhash 失败");
  ...
  for (const item of volumeItems) {
    tx.add(buildCloseVolumeIx(userPk, item)); // 关闭累加器，押金 + 返现全部释放给 user
  }
  const totalLamports = volumeItems.reduce((s, v) => s + (v.lamports || 0), 0);
  const feeLamports = Math.floor(totalLamports * VOLUME_FEE_RATE);
  const netLamports = totalLamports - feeLamports;
  // 收 10%：累加器押金只能先回 user（destination 固定），故同一交易内转 10% 给平台，原子、无需额外签名
  if (feeLamports > 0) {
    tx.add(SystemProgram.transfer({ fromPubkey: userPk, toPubkey: platformPk, lamports: feeLamports }));
  }
  return {
    serialized: tx.serialize({ requireAllSignatures: false, verifySignatures: false }).toString("base64"),
    accountCount: volumeItems.length,
    totalLamports,
